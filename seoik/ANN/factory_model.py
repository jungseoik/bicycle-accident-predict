import json
import pandas as pd
from ANN.cv import ClfiCV
from torch import nn, optim
from ANN.nn import DynamicANN, DynamicANNWrapper
...
def factory_model(feature, label, jsonstr):
    # 설정 파일 로드
...
def models_cv(models, feature, label, n_splits=5):
    ...
    for m in models:  # num_iterations는 반복 횟수
        current_cv_result = ClfiCV(m, feature, label, n_splits=5)  # CV 함수 실행
...
def create_models(feature, label, config_file, num_models):
    ...
    for i in range(1, num_models+1):
        ...
        locals()[model_variable_name] = factory_model(feature, label, config)
